...
def view_payslip_pdf(request, payslip_id):
    ...
    if Payslip.objects.filter(id=payslip_id).exists():
        ...
        if (
            ...
            # Extract employer contributions from saved deduction data
            if (
                "employer_contributions" not in data
                or not data["employer_contributions"]
            ):
                employer_contributions = []
                all_deduction_lists = [
                    data.get("basic_pay_deductions", []),
                ...
                for deduction_list in all_deduction_lists:
                    for deduction in deduction_list:
                        employer_amount = deduction.get(
                            "employer_contribution_amount", 0
                        )
                        employer_rate = deduction.get("employer_contribution_rate", 0)
                        if employer_amount > 0:
                            base_title = deduction.get(
                                "base_title", deduction.get("title")
                            )
                            title_with_rate = (
                                f"{base_title} ({employer_rate}%)"
                                if employer_rate
